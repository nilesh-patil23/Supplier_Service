		...
		try {
			...
			if (dbEntity == null) {
				...
				throw new ResourceAlreadyExistsException(
						"Category Already Exits with Id : " + categoryEntity.getCategoryId());

			}
		} catch (RollbackException e) {
			e.printStackTrace();
			throw new SomethingWentWrongException("Something went wrong in during add category");
		} finally {
			if (session != null) {
				session.close();
			}
	...
	public boolean updateCategory(CategoryEntity categoryEntity) {
		...
		try {
			...
			if (dbCategory != null) {
				...
				isUpdated = false;
				// throw new ResourceNotExistsException("Category Not Exists with ID : " +
